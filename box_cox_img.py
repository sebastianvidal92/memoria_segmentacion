...
class BoxCoxImg:
    ...
    def show(self):
        plt.imshow(self.img, cmap='gray' if len(self.img.shape) == 2 else None)
        plt.show()
    ...
    def black_white(self):
        # Si la imagen ya es en escala de grises (tiene 2 dimensiones), no la convierte
        if len(self.img.shape) == 2:
            return self.img
        else:
            # Conversión a escala de grises para imágenes con 3 canales (RGB)
